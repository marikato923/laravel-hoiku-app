        ...
        @foreach ($siblings as $index => $sibling)
            <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="content-{{ $sibling->id }}" 
                ...
                <div class="card">
                    ...
                    style="background-color: {{ optional($sibling->classroom)->theme_color ?? '#cccccc' }};">
                    ...
                    <div class="card-body">
                        <div class="row mb-3">
                            <div class="col-md-4">
                                {{-- 子供の画像 --}}
                                @php
                                    $themeColor = optional($sibling->classroom)->theme_color ?? '#ccc'; // クラスのテーマカラー（デフォルト：灰色）
                                @endphp
                                <div class="child-show-img-wrapper" style="border-color: {{ $themeColor }};">
                                    @if($sibling->img)
                                        <img src="{{ asset('storage/children/' . $sibling->img) }}" 
                                            alt="子供の写真" 
                                            class="child-img">
                                    @else
